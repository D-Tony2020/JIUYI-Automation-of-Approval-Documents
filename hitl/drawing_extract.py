...
【品号规则】这是给客户"生久"用的承认书,品号必须取【生久料号】——通常 YY 开头(如 YY60039403)。
图纸品号/料号栏可能同时印【生久料号 YY...】和【久益内部料号 SB.../其他】;务必取 YY 开头的生久料号,忽略 SB 开头的久益内部料号。

严格只输出JSON,无任何解释或markdown:
{"品号":"YY60039403","版本":"","名称":"","FAI尺寸":[{"标称":470,"上":8,"下":8}]}"""
...
def extract(drawing_pdf, api_key=None):
    ...
    for d in data.get("FAI尺寸", []):
        try:
            ...
            continue
    return {"品号": _pick_shengjiu_code(str(data.get("品号", "")).strip()),
            "版本": str(data.get("版本", "")).strip(),
            "名称": _clean_name(data.get("名称", "")),
            "dimensions": dims}


def _pick_shengjiu_code(raw):
    """品号优先取生久料号(YY开头)。LLM 若混入久益料号(SB...)或两者并列, 后处理锁定 YY\\d+。"""
    m = re.search(r"YY\d{4,}", raw, re.I)
    return m.group(0).upper() if m else raw


def _clean_name(raw):
    """名称=产品品类(导线), 去掉LLM混入的料号串(SB120420.../YY60039403)。golden 材质表A2/FAI品类均为'导线'。"""
    s = re.sub(r"[A-Za-z]+\d[A-Za-z0-9\-]*", "", str(raw or "")).strip()   # ASCII料号串(勿用\w, 会吞中文品类)
