...
import { groupByPart, detectDups, suspect, allMissing, materialMissing, cardState, syncPartSuppliers } from "./bomstate.js";
...
// 溯源标记: 字典映射推断出的值标"荐·待核对"(琥珀), 区别于MSDS直接提取的事实; 手改/核对即清。
function _markInf(o, f) { o.推断 = o.推断 || []; if (!o.推断.includes(f)) o.推断.push(f); }
function _clearInf(o, f) { if (o && o.推断) o.推断 = o.推断.filter((x) => x !== f); }
function _isInf(o, f) { return !!(o && Array.isArray(o.推断) && o.推断.includes(f)); }
function _confirmAll(m) { m.推断 = []; (m.成份 || []).forEach((c) => { c.推断 = []; }); }   // 标记通过=一键清该材质全部推断

// 材质为锚: 用 材质原文 反查字典, 自动填 标准名/类别/零件(仅未填时), 字典推断的标"荐"
function autoResolveAll() {
  for (const m of S.materials) {
    ...
    if (!(m.材质 || "").trim() || m.材质 === raw) {
      if (r.标准名 && r.标准名 !== raw) _markInf(m, "材质");      // 简称字典resolve出的标准名≠原文=推断
      m.材质 = r.标准名;
    }
    if (m.材质名来源 === "文件名兜底") _markInf(m, "材质");        // 文件名兜底的材质名=推断
    if (!(m.材质类别 || "").trim() && r.材质类别) { m.材质类别 = r.材质类别; _markInf(m, "材质类别"); }
    if (!(m.零件 || "").trim() && r.零件) { m.零件 = r.零件; _markInf(m, "零件"); }
    const lib = (S.dicts.comp || {})[m.材质];                    // 材质成份库预填: 本料一条成份都没抽到时按库补(整条=推断)
    if (lib && lib.length && !(m.成份 || []).length) {
      m.成份 = lib.map((c) => ({ 成份名称: c.成份名 || c.成份名称 || "", CAS: c.CAS || "",
                                "重量%": "", 无CAS: !c.CAS, 推断: ["成份名称", "CAS"] }));
    }
  }
}
...
function render() {
  syncPartSuppliers(S.materials);     // 自愈: 新增/拖入材质继承零件级供应商(见 bomstate.syncPartSuppliers)
  renderToolbar();
...
function card(i, dupOf) {
  ...
  return `<div class="card mat-card ${m.手补 ? "manual" : ""}" data-st="${st}" data-i="${i}">
    ...
    <div class="mat-row1">
      ${badge}
      <span class="inf-wrap">
        <input class="inp matname-in${_isInf(m, "材质") ? " inf" : ""}" data-mat="${i}" value="${esc(m.材质)}" title="${_isInf(m, "材质") ? "据字典推荐，请核对(非MSDS直接提取) · " : ""}原文:${esc(m.材质原文) || "—"} · 改为标准名→自动出类别/零件" placeholder="材质(标准名)">
        ${_isInf(m, "材质") ? '<span class="inf-badge" title="据字典推荐，请核对">荐</span>' : ""}
      </span>
      <select class="inp part-sel ${m.零件 ? "" : "need"}${_isInf(m, "零件") ? " inf" : ""}" data-part="${i}"${_isInf(m, "零件") ? ' title="据字典推荐，请核对"' : ""}>${partOpts}</select>
      <select class="inp ${m.材质类别 ? "" : "need"}${_isInf(m, "材质类别") ? " inf" : ""}" data-cat="${i}"${_isInf(m, "材质类别") ? ' title="据字典推荐，请核对"' : ""}>${catOpts}</select>
...
function block(i, reasons) {
  ...
  const rows = (m.成份 || []).map((c, j) =>
    `<tr class="${c.无CAS ? "nocas" : ""}">
      <td><input class="cmp${_isInf(c, "成份名称") ? " inf" : ""}" data-c="${i}|${j}|成份名称" value="${esc(c.成份名称)}"${_isInf(c, "成份名称") ? ' title="据字典推荐，请核对"' : ""}></td>
      <td><input class="cmp${_isInf(c, "CAS") ? " inf" : ""}" data-c="${i}|${j}|CAS" value="${esc(c.CAS)}" placeholder="${c.无CAS ? "无CAS?" : ""}"${_isInf(c, "CAS") ? ' title="据字典推荐(CAS规范库)，请核对"' : ""}></td>
...
function renderToolbar() {
  ...
  $("passallbom").onclick = () => { S.materials.forEach((m) => { if (!m.豁免) { m.已核对 = true; _confirmAll(m); } }); save(); render(); };  // 一键核对所有(非豁免)+清全部"荐"
...
function bind() {
  ...
  document.querySelectorAll("[data-cat]").forEach((el) => el.onchange = () => { const m = S.materials[+el.dataset.cat]; m.材质类别 = el.value; _clearInf(m, "材质类别"); logChange("改材质类别", `${(m.材质 || "材质")}: → ${el.value || "(空)"}`); save(); render(); });
...
function onPass(i) {                          // [标记通过]/[取消通过] — 独立于展开(展开由成分▸/▾或黄条控制)
  ...
  if (m.豁免) return;
  m.已核对 = !m.已核对;
  if (m.已核对) _confirmAll(m);              // 标记通过=一键核对该材质全部"荐"推断
  save(); render();
}

function setMat(i, val) {                    // 改材质名→反查字典自动填类别/零件; 材质本身手改=确认, 重派生的类别/零件=荐
  const m = S.materials[i];
  const old = m.材质 || "";
  m.材质 = val.trim();
  _clearInf(m, "材质"); delete m.材质名来源;   // 操作员手改材质名=核对确认
  const r = resolveMaterial(m.材质, S.dicts.alias, S.dicts.catpart);
  if (r.材质类别) { m.材质类别 = r.材质类别; _markInf(m, "材质类别"); }   // 改名后重派生=荐(请核对)
  if (r.零件) { m.零件 = r.零件; _markInf(m, "零件"); }
...
async function setPart(i, val) {
  ...
  _clearInf(S.materials[i], "零件");          // 手选零件=核对确认→清"荐"
...
function reorderMatTo(from, toIdx) {          // 拖材质卡到目标卡前: 同零件内调序(老板要的) / 跨零件移到该位置; 数组序==装表展开序→连续性
  ...
  const oldPart = (m.零件 || "").trim(), newPart = (target.零件 || "").trim();
  m.零件 = newPart;                            // 落到目标卡所在零件(同零件=纯排序; 异零件=移位并改归属)
  if (newPart !== oldPart) m.供应商 = "";      // 跨零件移动→丢旧供应商, 由 syncPartSuppliers 继承目标零件的(防把旧值带进新零件)
...
function moveMatToPart(i, p) {                // 拖动改材质归属: 改零件→传递到装表(stage2_to_nested_bom按零件分组+材质表/OLE跟随)
  ...
  m.供应商 = "";                              // 改零件归属→丢旧供应商, 由 syncPartSuppliers 继承目标零件(待认领则保持空)
  save(); render();
}
...
function editComp(token, val) {
  ...
  _clearInf(c, field);                                          // 手改该字段=核对确认→清"荐"
  const cn = S.dicts.casname || {};
  if (field === "CAS") {
    c.无CAS = !c.CAS || ["/", "-"].includes(c.CAS);
    if (cn[c.CAS] && !(c.成份名称 || "").trim()) { c.成份名称 = cn[c.CAS]; _markInf(c, "成份名称"); }   // CAS→规范名(自动带=荐, 仅空时填)
  }
  if (field === "成份名称" && c.成份名称 && !(c.CAS || "").trim()) {
    const cas = Object.keys(cn).find((k) => cn[k] === c.成份名称);          // 名→CAS(反查, 自动带=荐)
    if (cas) { c.CAS = cas; c.无CAS = false; _markInf(c, "CAS"); }
  }
  save(); render();
}
...
async function onConfirm() {
  syncPartSuppliers(S.materials);     // 提交前再自愈一次, 保证落库的 stage2 每材质供应商与零件级一致(后端硬门也按材质级查)
